...
class WomenAdmin(admin.ModelAdmin): # класс для регистрации и отображения записей таблицы women в админ панели
    fields = ['title', 'photo', 'post_photo', 'slug', 'content', 'cat', 'husband', 'tags'] # поля которые будут отображаться в форме редактирования(создания) записи
    ...
    list_display = ['title', 'post_photo', 'time_create', 'is_published', 'cat'] # какие поля отображать(ранее отображалось еще свое поле brief_info)
    ...
    save_on_top = True # для отображение панели сохранения в админ панели еще и сверху
    ...
    def brief_info(self, women: Women): # этот метод можем использовать в list_display для отображения еще одного поля
    ...
    def post_photo(self, women: Women): # для отображения фото в админ панели
        if women.photo:
            return mark_safe(f"<img src='{women.photo.url}' width=50>")  # mark_safe для того чтобы html теги не экранировались
        return 'Без фото'
